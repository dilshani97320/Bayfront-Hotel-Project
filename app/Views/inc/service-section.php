<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <title></title>
</head>
<body>
  <div class='information-container'>
  <div class='inner-container'>
   <!--  <h1 class='section-title'>Information</h1> -->
    <div class='border'></div>
      <div class='service-container'>
        
        <div class='service-box'>
          <div class='service-icon'>
            <!-- <i class='fa fa-paint-brush'></i> -->
            <img src="<?php echo BURL.'assets/img/gallery/service_block/room.png'; ?>" alt="">
          </div>
          <div class='service-title'>Cozy and Charming rooms</div>
          <div class='description'>
          </div>
        </div>
        
        <div class='service-box'>
          <div class='service-icon'>
            <!-- <i class='fa fa-code'></i> -->
            <img src="<?php echo BURL.'assets/img/gallery/service_block/cocktail.png'; ?>" alt="">
          </div>
          <div class='service-title'>Bar Drinks</div>
          <div class='description'>
          </div>
        </div>
        
        <div class='service-box'>
          <div class='service-icon'>
            <!-- <i class='fa fa-cut'></i> -->
            <img src="<?php echo BURL.'assets/img/gallery/service_block/beach.png'; ?>" alt="">
          </div>
          <div class='service-title'>Relax in a beautiful contest</div>
          <div class='description'>
          </div>
        </div>
        
        <div class='service-box'>
          <div class='service-icon'>
            <!-- <i class='fa fa-object-ungroup'></i> -->
            <img src="<?php echo BURL.'assets/img/gallery/service_block/time.png'; ?>" alt="">
          </div>
          <div class='service-title'>Exceptional Food</div>
          <div class='description'>
          </div>
        </div>
        
      </div>
    </div>
  </div>

</body>
</html>